...
export default async function DashboardPage() {
  ...
  const [
    ...
    supabase.from('assets').select('*, slugs(display_name, slug)').eq('is_archived', false).order('upload_date', { ascending: false }).limit(6),
    supabase.from('initiatives').select('*, slugs(display_name, slug)').in('status', ['active', 'ongoing']).order('created_at', { ascending: false }).limit(10),
